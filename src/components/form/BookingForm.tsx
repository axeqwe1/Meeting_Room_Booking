  ...
  const validation = (
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        const isoStart = DateTime.fromFormat(
          booking.start!!,
          "yyyy-MM-dd'T'HH:mm",
          { zone: "utc" }
        )
          .toUTC()
          .toISO();
        const isoEnd = DateTime.fromFormat(
          booking.end!!,
          "yyyy-MM-dd'T'HH:mm",
          { zone: "utc" }
        )
        ...
        const iniStartDate = isoStart
          ? new Date(isoStart)
          : initialDate
          ? new Date(initialDate)
          : new Date();

        const iniEndDate = isoEnd
          ? new Date(isoEnd)
          : initialEndDate
        ...
        if (
          !iniStartDate ||
          !iniEndDate ||
          isNaN(iniStartDate.getTime()) ||
          isNaN(iniEndDate.getTime())
        ) {
          return resolve({
            success: false,
        ...
        const wrongFormat: boolean =
          iniEndDate.getTime() <= iniStartDate.getTime();
        ...
        if (
          isAllDay(iniStartDate, iniEndDate) ||
          iniStartDate.toDateString() !== iniEndDate.toDateString()
        ) {
        ...
        const hasAllDayOrCrossDayBooking = bookData.some((booking) => {
          ...
          const bookingEnd = new Date(booking.end);
          if (
            isAllDay(bookingStart, bookingEnd) ||
        ...
        if (isOverlapping) {
          return resolve({
            success: false,
            message:
              "This room is already booked for the selected time period. Please choose a different time slot.",
          });
        }
        resolve({
          success: true,
            ...
            <div>
              ...
              <div className="mt-1 relative rounded-md shadow-sm">
                ...
                <input
                  ...
                  value={
                    booking.end
                      ? DateTime.fromISO(booking.end, { zone: "utc" }).toFormat(
                          "yyyy-MM-dd'T'HH:mm"
                        )
                      : ""
                  }
